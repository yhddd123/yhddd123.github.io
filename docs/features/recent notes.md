...
- Show a 'see more' link: pass in an additional parameter to `Component.RecentNotes({ linkToMore: "tags/components" })`. This field should be a full slug to a page that exists. The slug is the file path without the `.md` extension (e.g., `"content/recentnote.md"` → slug: `"recentnote"`, or `"content/笔记/组合数学.md"` → slug: `"笔记/组合数学"`).
